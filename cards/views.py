...
def build_search_query(query_raw: str, rarity=None, supertype=None, subtype=None, artist=None) -> str:
    ...
    if rarity: safe_append(query_parts, Rarity, rarity, "rarity", "name")
    if supertype: safe_append(query_parts, Supertype, supertype, "supertype", "name")
    if subtype: safe_append(query_parts, Subtype, subtype, "subtypes", "name")
    if artist: safe_append(query_parts, Artist, artist, "artist", "name")
...
def translate_query(query_raw: str) -> str:
    ...
    pokemon = PokemonEspecie.objects.filter(Q(name_es__icontains=q) | Q(name_en__icontains=q)).first()
    if pokemon: return pokemon.name_en
    if q in TCG_TERMS: return TCG_TERMS[q]
    if q in POKEMON_ES_TO_TCG: return POKEMON_ES_TO_TCG[q]
    for es, en in POKEMON_ES_TO_TCG.items():
        if normalize(es).startswith(q): return en
...
def get_filter_options(filter_name=None):
    ...
    if filter_name: return filters.get(filter_name)
    return filters
...
def resolve_card_relations(card_data: dict):
    ...
    if card_data.get('rarity'):
        rarity, _ = Rarity.objects.get_or_create(name=normalize(card_data['rarity']), defaults={'display_name': card_data['rarity']})
        relations['rarity'] = rarity
    if card_data.get('supertype'):
        supertype, _ = Supertype.objects.get_or_create(name=normalize(card_data['supertype']), defaults={'display_name': card_data['supertype']})
    ...
    if card_data.get('subtypes') and isinstance(card_data['subtypes'], list):
        ...
        if subtype_name:
            subtype, _ = Subtype.objects.get_or_create(name=normalize(subtype_name), defaults={'display_name': subtype_name})
    ...
    if card_data.get('name'):
        pokemon = PokemonEspecie.objects.filter(Q(name_en__icontains=card_data['name']) | Q(name_es__icontains=card_data['name'])).first()
        if pokemon: relations['pokemon_especie'] = pokemon
    return relations
...
def register(request):
    if request.method == 'POST':
        ...
        if password != password_confirm: return render(request, 'auth/register.html', {'error': 'Las contraseñas no coinciden'})
        if User.objects.filter(username=username).exists(): return render(request, 'auth/register.html', {'error': 'El usuario ya existe'})
...
def search(request):
    ...
    api_query = build_search_query(query_raw, selected_rarity_id, selected_supertype_id, selected_subtype_id, selected_artist_id)
    ...
    if query_raw or selected_rarity_id or selected_supertype_id or selected_subtype_id or selected_artist_id:
        ...
        if query_raw:
            ...
            name_q = Q()
            for term in target_terms: name_q |= Q(name__icontains=term)
            db_filters &= name_q
        
        # Validar numéricos para evitar ValueErrors ocultos
        if selected_rarity_id and str(selected_rarity_id).isdigit(): db_filters &= Q(rarity_id=selected_rarity_id)
        if selected_supertype_id and str(selected_supertype_id).isdigit(): db_filters &= Q(supertype_id=selected_supertype_id)
        if selected_subtype_id and str(selected_subtype_id).isdigit(): db_filters &= Q(subtype_id=selected_subtype_id)
        if selected_artist_id and str(selected_artist_id).isdigit(): db_filters &= Q(artist_id=selected_artist_id)

        db_results = Card.objects.filter(db_filters).select_related("rarity", "supertype", "subtype", "artist")[:150]
        for c in db_results:
        ...
        try:
            ...
            if api_data:
                for card in api_data:
                    cid = card.get("id")
                    if cid: unique_cards_map[cid] = card
    ...
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            ...
            local_cards = data_cargada.get("data", list(data_cargada.values())) if isinstance(data_cargada, dict) else data_cargada
            
            target_terms = get_expanded_search_terms(query_raw) if query_raw else set()
            target_rarity = Rarity.objects.filter(id=selected_rarity_id).first().name if selected_rarity_id and str(selected_rarity_id).isdigit() else None
            target_supertype = Supertype.objects.filter(id=selected_supertype_id).first().name if selected_supertype_id and str(selected_supertype_id).isdigit() else None
            target_subtype = Subtype.objects.filter(id=selected_subtype_id).first().name if selected_subtype_id and str(selected_subtype_id).isdigit() else None
            target_artist = Artist.objects.filter(id=selected_artist_id).first().name if selected_artist_id and str(selected_artist_id).isdigit() else None

            for card in local_cards:
                match = True
                if target_terms:
                    card_name = card.get("name", "").lower()
                    if not any(term in card_name for term in target_terms): match = False
                if target_rarity and normalize(card.get("rarity")) != normalize(target_rarity): match = False
                if target_supertype and normalize(card.get("supertype")) != normalize(target_supertype): match = False
                if target_artist and normalize(card.get("artist")) != normalize(target_artist): match = False
                if target_subtype:
                    subtypes_list = card.get("subtypes", [])
                    if isinstance(subtypes_list, str): subtypes_list = [subtypes_list]
                    if not any(normalize(st) == normalize(target_subtype) for st in subtypes_list): match = False
    ...
    if not unique_cards_map and not query_raw:
        try:
            ...
            for card in api_data: unique_cards_map[card.get("id")] = card
    ...
    if all_cards:
        if selected_sort in ["price", "-price"]:
            all_cards.sort(key=lambda x: float(extract_market_price(x.get("tcgplayer", {}).get("prices", {})) if x.get("tcgplayer") else (x.get("price") or 0.0)), reverse=(selected_sort == "-price"))
        ...
        for card_data in page_cards:
            ...
            if not card_formatted.get("image_url"): card_formatted["image_url"] = card_data.get("image_url") or card_data.get("images", {}).get("small", "")
            if not card_formatted.get("set_name") or card_formatted.get("set_name") == "Unknown": card_formatted["set_name"] = card_data.get("set_name") or card_data.get("set", {}).get("name", "Unknown")
            if not card_formatted.get("price"): card_formatted["price"] = extract_market_price(card_data.get("tcgplayer", {}).get("prices", {})) if card_data.get("tcgplayer") else card_data.get("price")

            if relations.get("rarity"): card_formatted["rarity"] = relations["rarity"].display_name
            if relations.get("artist"): card_formatted["artist"] = relations["artist"].name
            if relations.get("supertype"): card_formatted["supertype"] = relations["supertype"].display_name

            card_formatted["id"] = card_data.get("id") or card_data.get("pokemontcg_id") or card_data.get("card_id")
            results.append(card_formatted)
    else:
        total_pages = 1

    context = {
        "results": results, "error": error, "current_page": current_page, "total_pages": total_pages,
        "has_next": current_page < total_pages, "has_previous": current_page > 1, "next_page": current_page + 1, "prev_page": current_page - 1,
        "query_raw": query_raw, "selected_sort": selected_sort, "selected_supertype_id": selected_supertype_id,
        "selected_rarity_id": selected_rarity_id, "selected_subtype_id": selected_subtype_id, "selected_artist_id": selected_artist_id,
        "options": {
            "subtypes": get_filter_options("subtypes"), "rarities": get_filter_options("rarities"),
            "supertypes": get_filter_options("supertypes"), "artists": get_filter_options("artists"),
        }
    }
...
def search_suggestions(request):
    ...
    try:
        ...
        name_q = Q()
        for term in expanded_terms: name_q |= Q(name__icontains=term)
            
        cards_found = Card.objects.filter(name_q)[:100]
        for c in cards_found:
            all_cards.append({"name": c.name, "image_url": c.image_url, "set_name": c.set_name, "pokemontcg_id": c.pokemontcg_id})
            ...
            try:
                ...
                if api_data:
                    for api_card in api_data[:10]:
                        ...
                        all_cards.append({"name": formatted_api.get("name"), "image_url": formatted_api.get("image_url"), "set_name": formatted_api.get("set_name"), "pokemontcg_id": formatted_api.get("id")})
            except Exception: pass
        ...
        results = [{"name": c.get("name"), "image": c.get("image_url"), "image_url": c.get("image_url"), "set": c.get("set_name"), "pokemontcg_id": c.get("pokemontcg_id")} for c in unique_cards]
...
def create_alert(request):
    ...
    try:
        ...
        if not card:
            ...
            if card_data:
                ...
                card = Card.objects.create(
                    pokemontcg_id=pokemontcg_id, name=formatted.get("name"),
                    image_url=formatted.get("image_url") or formatted.get("images", {}).get("small", ""),
                    set_name=formatted.get("set_name"), number=formatted.get("number"), price=formatted.get("price") or current_price,
                    **relations
                )
            else:
        ...
        PriceAlert.objects.create(user=request.user, card=card, discount_percentage=discount, target_price=round(target_price, 2), is_active=True)
...
def edit_alert(request, alert_id):
    ...
    context = {'alert': alert, 'price_dates': [h.recorded_at.strftime('%Y-%m-%d') for h in historial], 'price_values': [float(h.price) for h in historial]}
    return render(request, 'alerts/edit_alert.html', context)


@login_required(login_url="login")
def card_detail(request, card_id):
    if not card_id: return render(request, "card_detail.html", {"card": {}, "market_price": "N/A", "error": "Carta no válida."})
    cache_key = f"card_detail_{card_id}"
    cached = cache.get(cache_key)
    if cached: return render(request, "card_detail.html", cached)

    error, card, market_price = None, {}, "N/A"
    card_obj = Card.objects.select_related("rarity", "supertype", "subtype", "artist", "pokemon_especie").filter(pokemontcg_id=card_id).first()

    if card_obj:
        price = float(card_obj.price or 0)
        card = {
            "id": card_obj.pokemontcg_id, "name": card_obj.name, "images": {"small": card_obj.image_url, "large": card_obj.image_url},
            "price": price, "rarity": card_obj.rarity.display_name if card_obj.rarity else "N/A", "artist": card_obj.artist.name if card_obj.artist else "Desconocido",
            "supertype": card_obj.supertype.display_name if card_obj.supertype else "N/A", "subtype": card_obj.subtype.display_name if card_obj.subtype else "N/A",
            "pokemon": card_obj.pokemon_especie.name_en if card_obj.pokemon_especie else None, "set": {"name": card_obj.set_name}, "number": card_obj.number,
        }
        market_price = f"{price:.2f}"
    else:
        try:
            data = fetch_card(card_id)
            if not data: error = "La carta no existe en la API."
            else:
                relations = resolve_card_relations(data)
                card = format_card(data)
                if relations.get("rarity"): card["rarity"] = relations["rarity"].display_name
                if relations.get("artist"): card["artist"] = relations["artist"].name
                if relations.get("supertype"): card["supertype"] = relations["supertype"].display_name
                if relations.get("subtype"): card["subtype"] = relations["subtype"].display_name
                if relations.get("pokemon_especie"): card["pokemon"] = relations["pokemon_especie"].name_en
                market_price = f"{card['price']:.2f}"
                Card.objects.get_or_create(pokemontcg_id=data["id"], defaults={"name": card["name"], "image_url": card["image_url"], "set_name": card.get("set_name"), "number": card.get("number"), "price": card.get("price"), **relations})
        except Exception as e: error = f"Error al obtener carta: {str(e)}"

    context = {"card": card, "market_price": market_price, "error": error}
    if not error: cache.set(cache_key, context, 60 * 60 * 24)
...
def card_price_history(_request, card_id):
    try:
        ...
        history = PriceHistory.objects.filter(card=card, recorded_at__gte=last_30_days).order_by('recorded_at')
        return JsonResponse({'dates': [h.recorded_at.strftime('%d/%m') for h in history], 'prices': [float(h.price) for h in history], 'card_name': card.name})
