...
def save_daily_prices():
    ...
    try:
        ...
        cards_with_alerts = Card.objects.filter(
        ...
        for card in cards_with_alerts:
            try:
                response = requests.get(
                    f"{TCG_API_URL}/{card.pokemontcg_id}",
                    headers=headers,
                    timeout=5
                )
                ...
                if response.status_code == 200:
                    ...
                    price = (
                        prices_data.get('holofoil', {}).get('market') or
                        prices_data.get('reverseHolofoil', {}).get('market') or
                        prices_data.get('normal', {}).get('market')
                    )
