...
def check_pokemon_prices():
    try:
        ...
        # 3. Alertas — se evalúan SIEMPRE contra el precio actual guardado en Card.price,
        # sin depender de si la carta se refrescó hoy o no. Así una alerta nunca se queda
        # "huérfana" esperando al día siguiente por culpa del dedupe de arriba.
        emails_sent = 0
        for alert in PriceAlert.objects.filter(is_active=True).select_related("card", "user"):
            live_price = alert.card.price
            if live_price is None:
                # Carta sin precio conocido todavía: no hay nada que evaluar.
                continue

            live_price = float(live_price)
            target = float(alert.target_price) if alert.target_price else live_price * 0.9
            ...
            if live_price <= target:
                try:
                    ...
                    logger.error(f"Error email para alerta {alert.id}: {e}")

        # 4. Limpieza de datos antiguos (>90 días)
        PriceHistory.objects.filter(recorded_at__lt=timezone.now() - timedelta(days=90)).delete()

        resultado = (
            f"Proceso finalizado. Nuevos precios: {saved_count} | "
            f"Errores: {error_count} | Emails enviados: {emails_sent}"
        )
...
def actualizar_pokedex_automatica():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            resultado = (
                f"Sincronización de Pokédex completada. Se añadieron {nuevos_count} especies."
            )
            logger.info(resultado)
            return resultado
        else:
